...
void GatewayServerNetWorkHandler::initAllMsgRegister() {
    // REGISTER_MSG_ID_FUN(INNER_TO_GAME_LOGIN_RES,InnerLoginResponse, PlayerLoginHandler::onLoginRes);
    // REGISTER_MSG_ID_FUN(INNER_SERVER_HAND_SHAKE_RES,InnerServerHandShakeRes, PlayerLoginHandler::onShakeHandRes);
}

void GatewayServerNetWorkHandler::startThread(int threadNum) {
    threadPool = new AthenaThreadPool();
    threadPool->create(threadNum);
}

void GatewayServerNetWorkHandler::onConnect(Channel *channel, int status) {
